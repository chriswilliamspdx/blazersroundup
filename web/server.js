...
const {
  ...
  WEB_BASE_URL,               // FIX: Add the base URL of THIS service. e.g., https://blazersroundup-production.up.railway.app
...
const sessionStore = {
  ...
  async del(sub) {
    await pg.query(`DELETE FROM oauth_sessions WHERE sub = $1`, [key])
  },
}

// ------------------------------
// Import private key and prepare keyset
// ------------------------------
...
// FIX: Construct the redirect URI from our actual service URL (WEB_BASE_URL)
// This MUST exactly match one of the URIs in your docs/bsky-client.json file.
const redirectUri = new URL('/oauth/callback', WEB_BASE_URL).toString()

// This metadata is used internally by the client library
...
// Step 2 of OAuth: callback for Bluesky OAuth
...
// Protected endpoint: internal API to post a thread
